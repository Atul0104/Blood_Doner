...
@Component({
  ...
  templateUrl:'./search-donar.html',
  styleUrls: ['./search-donar.css']
})
export class SearchDonar implements OnInit, OnDestroy  {
  ...
  //     bloodGroup: [''], // no validators, not required anymore
  //     city: ['']        // no validators, not required anymore
  //   });
  // }
  ...
  //   // Mock name list
  //   const names = [
  //     'Rahul Sharma', 'Priya Verma', 'Ankit Gupta', 'Neha Singh',
  //     'Rohit Kumar', 'Sneha Iyer', 'Vikas Yadav', 'Amit Patil',
  //     'Kiran Joshi', 'Meena Desai'
  //   ];
  ...
  //     'Thane', 'Solapur', 'Kolhapur', 'Amravati', 'Satara',
  //     'Jalgaon', 'Latur', 'Sangli'
  //   ];


  //   const getRandomUniqueNames = (count: number) => {
  //     const shuffled = [...names].sort(() => Math.random() - 0.5);
  //     return shuffled.slice(0, count);
  //   };


  //   const selectedNames = getRandomUniqueNames(3);


  //   const results = selectedNames.map(name => ({
  //     name: name,
  //     bloodGroup: bloodGroups[Math.floor(Math.random() * bloodGroups.length)],
  //     city: maharashtraCities[Math.floor(Math.random() * maharashtraCities.length)],
  //     phone: Math.floor(6000000000 + Math.random() * 3999999999).toString()
  //   }));

  //   setTimeout(() => {
  //     this.filteredDonors = results;
  //     this.loading = false;
  //   }, 500);
  // }
